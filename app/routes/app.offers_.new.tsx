  ...
  return {
    ...
    offer: offer
      ? {
          ...
          upsellType: offer.placement === "post_purchase" ? "post-purchase" : "pre-purchase",
...
export default function CreateOfferPage() {
  ...
  const upsellType =
    searchParams.get("type") === "post-purchase" ? "post-purchase" : offer?.upsellType ?? "pre-purchase";
  const typeLabel = upsellType === "post-purchase" ? "Post-Purchase" : "Pre-Purchase";
...
/* ---------------------------------------------------------------------
Update app/models/offer.server.ts createOffer to accept the new fields:
...
export async function createOffer(data: {
  ...
  return db.offer.create({
    data: {
      ...
      manualSelections: JSON.stringify(data.manualSelections),
    },
  });
}

In schema.prisma, store conditions/manualSelections as `String` (JSON text) unless
you're on Postgres with a Json column type — then use Json directly instead of
JSON.stringify. Run:
  npx prisma migrate dev --name add_conditions_and_manual_selections
--------------------------------------------------------------------- */